 ...
 */
    ...
    send_cfg_buf[0] = gtp_9271;
	cfg_info_len[0] =  CFG_GROUP_LEN(gtp_9271);
    ...
    // printk("[-----------waveshare-----------]   gtp_init_panel, abs_x_max=%d, abs_y_max=%d -------  1 \n", ts->abs_x_max, ts->abs_y_max);
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (!vcc_i2c) {
		vcc_i2c = regulator_get(&client->dev, "vcc3v3_lcd0_n");
		if (IS_ERR(vcc_i2c)) {
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ts->swap_xy = device_property_read_bool(&client->dev, "touchscreen-swapped-x-y");
    ts->inv_x = device_property_read_bool(&client->dev, "touchscreen-inverted-x");
    ts->inv_y = device_property_read_bool(&client->dev, "touchscreen-inverted-y");
    // printk("[-----------waveshare-----------]   goodix_ts_probe, swap_xy=%d, inv_x=%d, inv_y=%d  -------  1 \n", ts->swap_xy, ts->inv_x, ts->inv_y);
