...
static void gup_clk_count_init(s32 bCh, s32 bCNT)
{
	...
	buf = 0;
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _bRW_MISCTL__MEA_MODE, &buf,
			    1);
	//_bRW_MISCTL__MEA_SRCSEL = 8 + bCh; //From GIO1
	buf = 8 + bCh;
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _bRW_MISCTL__MEA_SRCSEL,
			    &buf, 1);
	//_wRW_MISCTL__MEA_MAX_NUM = bCNT; //Set the Measure Counts = 1
	buf = bCNT;
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _wRW_MISCTL__MEA_MAX_NUM,
			    &buf, 1);
...
static u32 gup_clk_count_get(void)
{
	...
	{
		gtp_i2c_read_bytes(gtp_i2c_connect_client, _bRO_MISCTL__MEA_RDY,
				   buf, 1);
		ready = buf[0];
	}
	...
	gup_bit_write(_fRW_MISCTL__MEA, 0, 0);
	gtp_i2c_read_bytes(gtp_i2c_connect_client, _dRO_MISCTL__MEA_VAL, buf,
			   4);
...
static u8 gup_clk_dac_setting(int dac)
{
	...
	gtp_i2c_read_bytes(gtp_i2c_connect_client, _wRW_MISCTL__RG_DMY, &buf1,
			   1);
	gtp_i2c_read_bytes(gtp_i2c_connect_client, _bRW_MISCTL__RG_OSC_CALIB,
			   &buf2, 1);

	buf1 = (buf1 & 0xFFCF) | ((dac & 0x03) << 4);
	buf2 = (dac >> 2) & 0x3f;

	gtp_i2c_write_bytes(gtp_i2c_connect_client, _wRW_MISCTL__RG_DMY, &buf1,
			    1);
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _bRW_MISCTL__RG_OSC_CALIB,
			    &buf2, 1);
...
static void gup_sys_clk_init(void)
{
	...
	buf = 0;
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _bRW_MISCTL__RG_LDO_A18_PWD,
			    &buf, 1);
	//_bRW_MISCTL__RG_BG_PWD = 0; //DrvMISCTL_BG_PowerON
	buf = 0;
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _bRW_MISCTL__RG_BG_PWD,
			    &buf, 1);
	//_bRW_MISCTL__RG_CLKGEN_PWD = 0; //DrvMISCTL_CLKGEN_PowerON
	buf = 0;
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _bRW_MISCTL__RG_CLKGEN_PWD,
			    &buf, 1);
	...
	buf = 1;
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _bRW_MISCTL__OSC_CK_SEL,
			    &buf, 1);
}
...
s32 gtp_gup_clk_calibration(void)
{
	...
	gtp_i2c_write_bytes(gtp_i2c_connect_client, _rRW_MISCTL__SWRST_B0_,
			    &buf, 1);
				...
				s32 start_index, s32 chk_len)
{
	...
	while ((chk_len > 0) && (chked_times < GTP_CHK_FW_MAX)) {
		...
		if ((FW_HEAD_LENGTH + start_index + cmp_len) >
		    sizeof(gtp_default_FW_fl)) {
			GTP_ERROR("Check failed, buffer overflow\n");
			break;
		}
		ret = gtp_i2c_read_bytes(client, cmp_addr, chk_cmp_buf,
					 cmp_len);
		if (ret < 0) {
			chk_fail = 1;
			break;
...
s32 gtp_gup_fw_download_proc(void *dir, u8 dwn_mode)
{
	...
	if (NULL == dir) {
		if (GTP_FL_FW_BURN == dwn_mode) // GT9XXF firmware burn mode
		{
			GTP_INFO("[fw_download_proc]Begin fw download ......");
			   ...
			   dwn_mode) // GTP_FL_ESD_RECOVERY: GT9XXF esd recovery mode
		{
			GTP_INFO(
	...
	ret = gup_check_update_file_fl(gtp_i2c_connect_client, &fw_head,
				       (char *)dir);
	show_len = 10;
